...
const LocalStorageDisplay = ({localObject}) => {
  //console.log('This is localObject: ', localObject)
  //const object = {coffeeZ: 'cup', chicken: 'eggsf', number: 33}
  // localStorage.setItem('Palaver', JSON.stringify(object));

  const localStorageObject = localObject.companyName ? localObject
                                                     : inputObjectTemplate;
  ...
  return (
    <div>
      ...
      {Object.keys(localStorageObject).map((localStorageObjectKey, i) => 
        <LocalStorageTextBox key={i} localStorageObjectKey={localStorageObjectKey} localStorageObject={localStorageObject} />
      )}
    </div>
    </div>
  )
